  ...
  estimate(matter) {
    ...
    if (matter.gt(DC.E9E15)) {
      if (planckedLogMatter.gt(this.proton)) {
        ...
        return [`반물질 양의 모든 자릿수가 플랭크 부피라면,
          ${logScale.name} ${logAmount}개 ${logScale.verb} 수 있습니다.`];
      }
      const logScale = this.microScale(logMatter);
      return [`반물질 양의 모든 자릿수가
        ${format(this.proton.div(logScale.amount).div(logMatter), 2, 1)} ${logScale.name}라면,
        양성자 하나를 만들 수 있습니다.`];
    }
    ...
    if (planckedMatter.gt(this.proton)) {
      ...
      return [`반물질 하나하나가 플랭크 부피라면,
        ${scale.name} ${amount}개 ${scale.verb} 수 있습니다.`];
    }
    const scale = this.microScale(matter);
    return [`반물질 하나하나가 ${format(this.proton.div(scale.amount).div(matter), 2, 1)} ${scale.name}라면,
      양성자 하나를 만들 수 있습니다.`];
  },
