...
export default {
  ...
  computed: {
    ...
    costDisplay() {
      ...
      return `반물질 ${formatPostBreak(InfinityDimension(this.tier).amRequirement)}에 도달`;
    },
    continuumString() {
    ...
    capTooltip() {
      if (this.enslavedRunning) return `The Nameless Ones가 무한 차원을 ${format(10)}개 넘게 구매하지 못하게 합니다`;
