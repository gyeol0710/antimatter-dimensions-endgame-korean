export const divinityMilestones = {
  firstDivine: {
    ...
    get reward() {
      return ` 반물질 ${format(Decimal.pow10(1e150))} 및 ${format(Decimal.pow10(1e225))} 소프트캡이 반물질 ${format(DC.E9E15)}에서 적용되고 신성마다 강해지는 소프트캡으로 대체됩니다
        ...
        모든 펠레 업그레이드를 구매한 상태로 파멸을 시작합니다
  ...
  divineDimensions: {
    ...
    get reward() {
      return ` 신성 차원을 해금합니다
        ...
        모든 하드론/암흑 하드론 효과가 상한에 도달하는 시간을 ${formatPercents(0.2)}만큼 줄입니다
        알파 붕괴의 잔재가 상한에 도달하는 시간을 ${formatPercents(0.2)}만큼 줄입니다
  ...
  hadronEmpowerment: {
    divinities: 3,
    get reward() {
      return ` ${formatInt(30)}초 이내에 라이텔라의 현실을 비활성화하는 데 필요한 양보다 반물질이 많으면 자동으로 불안정화합니다. 이 시간은 온전히 남은 차원 수로 ${formatInt(8)}을 나눈 값만큼 제곱근이 적용됩니다
        ${formatInt(8)}개 차원이 모두 비활성화되면 라이텔라의 현실을 자동으로 강입자화합니다
  ...
  pelleQoL: {
    divinities: 4,
    get reward() {
      return ` 무한/무한 돌파 업그레이드를 구매/충전한 상태로 펠레를 시작합니다
        펠레에 진입하면 균열이 자동으로 ${formatPercents(1)}까지 채워집니다
        은하 생성기의 상한을 해제한 뒤 균열이 ${formatX(10)} 더 빠르게 채워집니다
        신성 차원에 ${formatPow(1.05, 2, 2)}을 적용합니다
        알파 붕괴의 잔재가 상한에 도달하는 시간을 ${formatPercents(0.5)}만큼 줄입니다
  ...
  celestialSurge: {
    ...
    get reward() {
      return ` 헵터랙트 효과가 셀레스티얼 물질 오염 강도에도 적용됩니다
        ...
        에파리그 레벨 ${formatInt(40)}이 글리프 불안정성의 처음 ${formatInt(3)}단계 대신 처음 ${formatInt(5)}단계에 적용됩니다`;
    }
  },
  finalRebirth: {
    divinities: 7,
    get reward() {
      return ` 여섯 번째 글리프 불안정성 시작점이 완화됩니다
        ...
        알파 붕괴의 잔재가 상한에 도달하는 시간을 ${formatPercents(0.25)}만큼 줄입니다
